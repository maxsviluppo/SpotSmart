...
export default function App() {
  ...
  const handleToggleAll = (active: boolean, category?: string) => {
    ...
    updated.forEach(async (s) => {
      if ((!category || s.cat === category) && s.id) {
        try {
          await updateDoc(doc(db, 'news_sources', s.id), { active });
        } catch (e) {
          console.error("Firestore update failed during bulk toggle:", e);
        }
      }
    });
  };
      ...
      >
        <div className="relative w-full h-full group" ref={containerRef}>
          ...
          <div className="absolute top-6 left-6 z-[100]">
            <a 
              href="https://www.piazzacardarelli.com" 
              target="_blank" 
              rel="noopener noreferrer"
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 backdrop-blur-md border border-white/10 hover:bg-white/10 transition-all text-white/80 hover:text-white group/testata shadow-lg"
            >
              <Globe className="w-4 h-4 text-indigo-400 group-hover/testata:rotate-12 transition-transform" />
              <span className="text-[10px] font-bold tracking-widest uppercase">
                www.piazzacardarelli.com
              </span>
            </a>
          </div>
          ...
          <div className="absolute bottom-[30px] right-10 z-[100] flex items-center gap-4">
            ...
            <div className="relative">
              ...
              <motion.div
                ...
                transition={{ type: "spring", stiffness: 400, damping: 25 }}
                className={`w-14 h-14 rounded-full flex items-center justify-center border transition-all shadow-2xl backdrop-blur-md z-[100] p-0 overflow-hidden cursor-pointer ${isMenuOpen ? 'bg-white/20 border-white/40' : 'bg-white/5 border-white/10 hover:bg-white/10 opacity-90 hover:opacity-100'}`}
                onClick={() => {
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                      <header className="mb-12 pb-8 border-b border-white/5 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="bg-emerald-500/10 border border-emerald-500/20 px-6 py-3 rounded-2xl flex items-center gap-3">
                            ...
                            <span className="text-xs font-black text-white uppercase tracking-widest">{newsSources.length} Fonti Totali</span>
                          </div>
                          <button 
                            onClick={() => {
                          ...
                          >
                            <CheckCircle2 className={`w-4 h-4 transition-transform group-hover:scale-110 ${newsSources.every(s => s.active !== false) ? 'text-indigo-400' : 'text-white/40'}`} />
                      ...
                      <div className="bg-slate-900/60 border border-white/10 rounded-3xl p-8 mb-12 shadow-2xl">
                        ...
                        <button 
                          type="button" 
                          onClick={addSource} 
                          className="mt-8 w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-5 rounded-2xl transition-all uppercase tracking-widest text-[11px] shadow-xl shadow-indigo-600/20 active:scale-[0.98]"
                        >
                            ...
                            <div key={cat.id} className="relative">
                              <div className="flex items-center gap-4 mb-6">
                                ...
                                <button 
                                  ...
                                  className="text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-white/60 hover:text-white transition-all whitespace-nowrap"
                                >
                                  {newsSources.filter(s => s.cat === cat.label).every(s => s.active !== false) ? 'Disattiva Categoria' : 'Attiva Categoria'}
                                </button>
                              </div>
                              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                {catSources.map(source => (
                                  <div 
                                    ...
                                    className="bg-slate-900/30 border border-white/5 rounded-2xl p-5 hover:bg-slate-900 cursor-pointer transition-all group flex items-center justify-between"
                                  >
